...
#đọc file
doc = pd.read_excel(r"C:\Users\ACER\Documents\Phân tích dữ liệu với Python\honda_sell_data.xlsx")
print(doc)

#tải file excel
workbook = load_workbook(filename =r"C:\Users\ACER\Documents\Phân tích dữ liệu với Python\honda_sell_data.xlsx")
#chọn sheet mặc định
...
def lay_gt_cot_khong_lap():
    cot=input('nhập tên cột lây GT không lặp: ')
    loc = doc[cot].unique()
    for value in loc:
        print(value)
...
def tong_mean():
    ...
    tong = doc[cot].sum()
    print("Tổng ",cot, là:",tong)
    trung_binh = doc[cot].mean()
    print("giá trị trung bình của cột",cot ,"là: ",trung_binh)



# def phuong_sai():
#     cot = input("nhập tên cột cần tính: ")
#     trungbinh = doc[cot].mean
#     print('trung bình:',cot,trungbinh
#     print("độ lêchj so với giá trị trung bình")
ps = doc['Price'].var
print(ps)

def tu_phan_vi(doc):
    cot = input('nhập cột: ')
    print('tứ phân vị 1: ', doc[cot].quantile(0.25))
    print('tứ phân vị 2: ', doc[cot].quantile(0.5))
    print('tứ phân vị 3: ', doc[cot].quantile(0.75))
